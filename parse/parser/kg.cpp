...
void KG::FindOneHopParents(hash_t c_hash, hash_t p_hash, hash_t prevNode){
        ...
        for (auto &it: KGEdgeTable){
                ...
                if (edge->n2_hash == c_hash) {
                        if (prevNode != NULL) {
                                if (edge->n1_hash == prevNode) continue;
                        }
...
void KG::FindInteractiveEntities(hash_t startNode, hash_t prevNode, hash_t currNode, int8_t depth, std::set<hash_t> &visited, uint64_t minTime) {
        ...
        for (auto edge : childEdges) {
                ...
                if (visited.count(edge->n2_hash) == 0) {
                        ...
                        FindInteractiveEntities(startNode, currNode, edge->n2_hash, depth - 1, visited, std::stoll(edge->timestamp));
